...
// print_r($this->atts['id']);
$display_label = isset($this->skin_options['display_label']) ? $this->skin_options['display_label'] : false;
$reason_for_cancellation = isset($this->skin_options['reason_for_cancellation']) ? $this->skin_options['reason_for_cancellation'] : false;
?>
<div class="mec-wrap <?php echo esc_attr($event_colorskin); ?>">
    <div class="programme-carousel-container mec-event-carousel-<?php echo esc_attr($this->style); ?>">
        <?php
        ...
        <div class='mec-owl-crousel-skin-<?php echo esc_attr($carousel_type); ?> mec-owl-carousel mec-owl-theme'>
            ...
            foreach ($this->events as $date) :
                foreach ($date as $event) :
                    ...
                    // Fix for "Carousel View - Present" shortcode (id 778) showing future events. We want to limit to today.
                    if ($this->atts['id'] == 778) {
                        // print_r("Target Shortcode");
                        $today = date('Ymd');
                        $eStart = date('Ymd', strtotime($event_start_date));
                        $eEnd = date('Ymd', strtotime($event_end_date));
                        // echo $today . "=" . $eStart . "-" . $eEnd;
                        if ($eStart <= $today && $eEnd >= $today) {
                            // echo "Hit";
                            // Show events normally below if today is in the event date
                        } else {
                            // else skip this iteration in foreach
                            continue;
                        }
                    }
            ?>
                    <article class="programme-carousel <?php echo (isset($event->data->meta['event_past']) and trim($event->data->meta['event_past'])) ? 'mec-past-event ' : ''; ?> <?php echo esc_attr($multiple_date); ?> mec-clear <?php echo esc_attr($this->get_event_classes($event)); ?>" itemscope>
                        ...
                        <?php if ($this->style == 'type1') : ?>
                            ...
                            <div class="mec-event-carousel-content">
                                ...
                                <p class="mec-carousel-event-location"><span><?php echo (isset($location['name']) ? esc_html($location['name']) : '');
                                                                                echo (isset($location['address']) ? '</span><br>' . esc_html($location['address']) : ''); ?></p>
                        ...
                        <?php elseif ($this->style == 'type2') : ?>
                            <!-- <div class="event-carousel clearfix"> -->
                            <!-- <div class="event-carousel-type2-head clearfix"> -->
                            <div class="mec-event-image programme-img">
                                <?php
                                if ($event->data->thumbnails['tileview']) echo MEC_kses::element($this->display_link($event, $event->data->thumbnails['large'], ''));
                                else echo '<img src="' . $this->main->asset('img/no-image.png') . '" />';
                                ?>
                                <?php echo MEC_kses::element($this->get_label_captions($event)); ?>
                            </div>
                            <div class="mec-event-carousel-content-type2 programme-content">

                                <h4 class="mec-event-carousel-title title"><?php echo MEC_kses::element($this->display_link($event)); ?><?php echo MEC_kses::element($this->display_custom_data($event)); ?><?php echo MEC_kses::element($soldout); ?></h4>
                                <?php
                                // Category
                                foreach ($event->data->categories as $category) { ?>
                                    <div class="mec-events-event-categories categories"><a href="<?php echo get_term_link($category['id'], 'mec_category') ?>" class="mec-color-hover" rel="tag"><?php echo MEC_kses::element(esc_html($category['name'])) ?></a></div>
                                    <!-- <?php echo '<dd class="mec-events-event-categories"><a href="' . get_term_link($category['id'], 'mec_category') . '" class="mec-color-hover" rel="tag">' . MEC_kses::element($icon . esc_html($category['name']) . $color_html) . '</a></dd>'; ?>-->
                                <?php } ?>

                                <?php if (isset($settings['multiple_day_show_method']) && $settings['multiple_day_show_method'] == 'all_days') : ?>
                                    <span class=" mec-event-date-info date"><?php echo esc_html($this->main->date_i18n($this->date_format_type2_1, strtotime($event->date['start']['date']))); ?></span>
                                <?php else : ?>
                                    <span class="mec-event-date-info date"><?php echo MEC_kses::element($this->main->dateify($event, $this->date_format_type2_1)); ?></span>
                                <?php endif; ?>
            ...
            <?php
            // Add empty events if total events less than 4
            while ($items < 4) { ?>

                <article class="programme-carousel empty mec-clear ">
                    <div class="event-carousel-type4-head clearfix">
                        <div class="mec-event-image">
                        </div>
                        <!-- <div class="mec-event-overlay"></div> -->
