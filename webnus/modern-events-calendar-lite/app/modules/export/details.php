...
<ul class="export">
    <?php if ($settings['sn']['googlecal']) : ?><li><a class="" href="https://calendar.google.com/calendar/render?action=TEMPLATE&text=<?php echo urlencode($title); ?>&dates=<?php echo gmdate('Ymd\\THi00\\Z', ($start_time - $gmt_offset_seconds)); ?>/<?php echo gmdate('Ymd\\THi00\\Z', ($end_time - $gmt_offset_seconds)); ?>&details=<?php echo urlencode($description); ?><?php echo esc_attr($location); ?><?php echo (trim($rrule) ? '&recur=' . urlencode($rrule) : ''); ?>" target="_blank"><?php echo esc_html__('Add to Google Calendar', 'modern-events-calendar-lite'); ?></a></li><?php endif; ?>
    <?php if ($settings['sn']['ical']) : ?><li><a class="" href="<?php echo esc_url($this->ical_URL($event->data->ID, $occurrence)); ?>"><?php echo esc_html__('iCal/Outlook export', 'modern-events-calendar-lite'); ?></a></li><?php endif; ?>
</ul>